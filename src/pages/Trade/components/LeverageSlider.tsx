...
export const LeverageSlider = ({ value, setLeverage, maxLeverage = 100 }: Props) => {
  const leveragePoints = [0.1, 10, 20, 40, 60, 80, 100];
  
  // const calculatePosition = (point: number) => {
  //   const percentage = (point / maxLeverage) * 100;
  //   return `calc(${percentage}% - ${(percentage / 100) * 4}px)`;
  // };
  ...
  return (
    <div className="space-y-3 px-1">
      <div className="relative">
        {/* Track container */}
        <div className="relative w-full h-1 bg-gray-700 rounded-full mx-1 mt-6 mb-1"> {/* Adjusted margins */}
          {/* Active Track */}
          <div 
            className="absolute h-full bg-[#0300AD] rounded-full"
            style={{ 
              width: `calc(${(value / maxLeverage) * 100}% - ${((value / maxLeverage) * 100 / 100) * 4}px)`,
              left: '2px'
            }}
          />
        ...
        <div className="relative w-full -mt-1"> {/* Negative margin to pull up */}
          {leveragePoints.map((point) => {
            const position = (point / maxLeverage) * 100;
            return (
              <span
                key={point}
                className={`absolute text-xs ${value >= point ? 'text-white font-medium' : 'text-gray-400'}`}
                style={{
                  left: `calc(${position}% - ${(position / 100) * 4}px)`,
                  transform: 'translateX(-50%)',
        ...
        <div 
          ...
          style={{ 
            left: `calc(${(value / maxLeverage) * 100}% - ${((value / maxLeverage) * 100 / 100) * 4}px)`,
